...
function Reports({ data }) {
  ...
  const reportRows = useMemo(() => {
    ...
    selectedSheetRows.forEach(row => {
      const name =
        String(row.name || "Unknown").trim() || "Unknown";
    ...
    selectedAccuracyRows.forEach(row => {
      ...
      if (row.accuracy != null && row.accuracy !== "") {
        map[name].accuracyValues.push(
          Number(row.accuracy) || 0
        );
      }

      if (row.score != null && row.score !== "") {
        map[name].scoreValues.push(
          Number(row.score) || 0
        );
      }
    });
    ...
    return Object.values(map)
      .map(row => ({
        ...
        score: row.scoreValues.length
          ? row.scoreValues.reduce(
              (sum, value) => sum + value,
              0
            ) / row.scoreValues.length
          : 0,
      ...
      .sort(
        (a, b) =>
          (b.images + b.dailyCount) -
          (a.images + a.dailyCount)
      );
  ...
  const summary = useMemo(() => {
    ...
    selectedSheetRows.forEach(row => {
      if (row.name) {
        people.add(String(row.name).trim());
      }
      ...
      if (
        project &&
        !["Saturday", "Sunday", "On Leave"].includes(project)
      ) {
  ...
  const allDates = useMemo(
    () =>
      [
        ...(Array.isArray(data?.sheetRecords)
          ? data.sheetRecords.map(row =>
              normalizeDateValue(row.date)
            )
          : []),

        ...(Array.isArray(data?.accuracyRecords)
          ? data.accuracyRecords.map(row =>
              normalizeDateValue(row.date)
            )
          : [])
      ]
  ...
  function exportExcel() {
    ...
    downloadBlob(
      output,
      `AnnotatePro-${reportTitle.replace(
        /\s+/g,
        "-"
      )}.xlsx`,
  ...
  return (
    ...
    >
      ...
      <div className="report-print-area">
        ...
        <div className="grid two">
          <Panel title="QA summary">
            <div className="qa-grid">
              ...
              <div>
                <h2>
                  {summary.dailyCount.toLocaleString()}
                </h2>
                <p className="muted">
                  Accuracy daily count
                </p>
        ...
        <Panel
          title={`Team performance — ${range.label}`}
        >
          {!reportRows.length ? (
            <p className="muted">
              No report data exists for{" "}
              <b>{range.label}</b>.
            </p>
          ) : (
            <div className="table-wrap">
        ...
        <Panel
          title={`Report details — ${range.label}`}
        >
          <div className="qa-grid">
            <div>
              <h2>{summary.people}</h2>
              <p className="muted">
                Team members
              </p>
            </div>
            ...
            <div>
              <h2>
                {summary.reviews.toLocaleString()}
              </h2>
              <p className="muted">
                Reviews
              </p>
