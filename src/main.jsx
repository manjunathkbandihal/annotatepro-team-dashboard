...
/* =========================================================
   REPORTS
   Uses the existing imported Daily Effort + Accuracy Report data.
   Does not alter Team / Projects / QA / Analytics / Sheet Import data.
========================================================= */
function Reports({ data }) {
  ...
  const selectedSheetRows = Array.isArray(data?.sheetRecords)
    ? data.sheetRecords.filter(row => isDateInRange(row.date, range))
    : [];

  const selectedAccuracyRows = Array.isArray(data?.accuracyRecords)
    ? data.accuracyRecords.filter(row => isDateInRange(row.date, range))
    : [];
  ...
  const reportRows = useMemo(() => {
    ...
    selectedSheetRows.forEach(row => {
      ...
      map[name].images += worked;
      if (/review/i.test(String(row.type || ""))) {
        map[name].reviews += worked;
      }
    });

    selectedAccuracyRows.forEach(row => {
      const name = String(row.name || "Unknown").trim() || "Unknown";
      if (!map[name]) {
        map[name] = {
          name,
      ...
      map[name].dailyCount += Number(row.dailyCount) || 0;
      map[name].tp += Number(row.tp) || 0;
      map[name].fp += Number(row.fp) || 0;
      map[name].fn += Number(row.fn) || 0;

      if (row.accuracy != null && row.accuracy !== "") {
        map[name].accuracyValues.push(Number(row.accuracy) || 0);
      }
      if (row.score != null && row.score !== "") {
        map[name].scoreValues.push(Number(row.score) || 0);
      }
    });
    ...
    return Object.values(map)
      .map(row => ({
        ...row,
        accuracy: row.accuracyValues.length
          ? row.accuracyValues.reduce((sum, value) => sum + value, 0) / row.accuracyValues.length
          : 0,
        score: row.scoreValues.length
          ? row.scoreValues.reduce((sum, value) => sum + value, 0) / row.scoreValues.length
          : 0,
        errors: row.fp + row.fn
      }))
      .sort((a, b) => (b.images + b.dailyCount) - (a.images + a.dailyCount));
  }, [selectedSheetRows, selectedAccuracyRows]);
  ...
  const summary = useMemo(() => {
    ...
    selectedSheetRows.forEach(row => {
      if (row.name) people.add(String(row.name).trim());
      const project = String(row.project || "").trim();
      if (project && !["Saturday", "Sunday", "On Leave"].includes(project)) {
    ...
    const images = selectedSheetRows.reduce(
      (sum, row) => sum + (Number(row.worked) || 0),
      0
    );
    const reviewsFromSheet = selectedSheetRows.reduce(
      (sum, row) => /review/i.test(String(row.type || ""))
        ? sum + (Number(row.worked) || 0)
        : sum,
      0
    );
    const dailyCount = selectedAccuracyRows.reduce(
      (sum, row) => sum + (Number(row.dailyCount) || 0),
      0
    );
    const tp = selectedAccuracyRows.reduce(
      (sum, row) => sum + (Number(row.tp) || 0),
      0
    );
    const fp = selectedAccuracyRows.reduce(
      (sum, row) => sum + (Number(row.fp) || 0),
      0
    );
    const fn = selectedAccuracyRows.reduce(
      (sum, row) => sum + (Number(row.fn) || 0),
      0
    );
    const denominator = tp + fp + fn;
    const accuracy = denominator > 0
      ? (tp / denominator) * 100
      : selectedAccuracyRows.length
        ? selectedAccuracyRows.reduce((sum, row) => sum + (Number(row.accuracy) || 0), 0) / selectedAccuracyRows.length
        : 0;
    const score = selectedAccuracyRows.length
      ? selectedAccuracyRows.reduce((sum, row) => sum + (Number(row.score) || 0), 0) / selectedAccuracyRows.length
      : 0;
    ...
    return {
      ...
      images,
      reviews: reviewsFromSheet || dailyCount,
      dailyCount,
      tp,
      fp,
      fn,
  ...
  const projectSummary = useMemo(() => {
    ...
    selectedSheetRows.forEach(row => {
      const project = String(row.project || "").trim();
      if (!project || ["Saturday", "Sunday", "On Leave"].includes(project)) return;

      if (!map[project]) {
        map[project] = { project, images: 0, people: new Set() };
      }

      map[project].images += Number(row.worked) || 0;
      if (row.name) map[project].people.add(String(row.name).trim());
    });
  ...
  const allDates = useMemo(() => [
    ...(Array.isArray(data?.sheetRecords) ? data.sheetRecords.map(row => normalizeDateValue(row.date)) : []),
    ...(Array.isArray(data?.accuracyRecords) ? data.accuracyRecords.map(row => normalizeDateValue(row.date)) : [])
  ...
  function setPreset(type) {
    ...
    const latestDateObj = new Date(`${latest}T00:00:00`);
    let start = latest;
    let end = latest;
    ...
    if (type === "weekly") {
      ...
      start = toISODate(d.getFullYear(), d.getMonth() + 1, d.getDate());
    } else if (type === "monthly") {
      const d = new Date(latestDateObj);
      start = toISODate(d.getFullYear(), d.getMonth() + 1, 1);
    }

    const availableStart = allDates.find(date => date >= start) || latest;

    setReportType(type);
    setPeriod({
      mode: type === "daily" ? "single" : "range",
      start: type === "daily" ? latest : availableStart,
      end: latest
    });
  }
  ...
  function exportExcel() {
    const rows = reportRows.map(row => ({
      Date: range.label,
      Member: row.name,
      "Images Worked": row.images,
      Reviews: row.reviews,
      "Accuracy Report Count": row.dailyCount,
      Accuracy: row.accuracy ? `${row.accuracy.toFixed(1)}%` : "",
      Score: row.score ? row.score.toFixed(1) : "",
      Errors: row.errors
    }));
    ...
    const summarySheet = XLSX.utils.json_to_sheet([{
      Report: reportTitle,
      Period: range.label,
      "Team Members": summary.people,
      Projects: summary.projects,
      "Images Worked": summary.images,
      Reviews: summary.reviews,
      "Accuracy Daily Count": summary.dailyCount,
      Accuracy: `${summary.accuracy.toFixed(1)}%`,
      "True Positives": summary.tp,
      "False Positives": summary.fp,
      "False Negatives": summary.fn,
      "Total Errors": summary.errors,
      "Average Score": summary.score.toFixed(1)
    }]);

    const memberSheet = XLSX.utils.json_to_sheet(rows);
    const projectSheet = XLSX.utils.json_to_sheet(projectSummary.map(row => ({
      Project: row.project,
      "Images Worked": row.images,
      "Team Members": row.people
    })));

    XLSX.utils.book_append_sheet(workbook, summarySheet, "Summary");
    XLSX.utils.book_append_sheet(workbook, memberSheet, "Team Performance");
    XLSX.utils.book_append_sheet(workbook, projectSheet, "Projects");

    const output = XLSX.write(workbook, { bookType: "xlsx", type: "array" });
    downloadBlob(
      output,
      `AnnotatePro-${reportTitle.replace(/\s+/g, "-")}.xlsx`,
  ...
  function exportCSV() {
    ...
    downloadBlob(
      lines.join("\n"),
      `AnnotatePro-${reportTitle.replace(/\s+/g, "-")}.csv`,
  ...
  return (
    <Page
      title="Reports"
      subtitle="Generate daily, weekly, monthly and custom operational reports from your imported dashboard data."
    >
      <div className="report-toolbar">
        <div className="report-type-buttons">
          <button type="button" className={reportType === "daily" ? "active" : ""} onClick={() => setPreset("daily")}>Daily</button>
          <button type="button" className={reportType === "weekly" ? "active" : ""} onClick={() => setPreset("weekly")}>Weekly</button>
          <button type="button" className={reportType === "monthly" ? "active" : ""} onClick={() => setPreset("monthly")}>Monthly</button>
          <button type="button" className={reportType === "custom" ? "active" : ""} onClick={() => { setReportType("custom"); setPeriod(p => ({ ...p, mode: "range" })); }}>Custom</button>
        </div>

        <div className="report-actions">
          <button type="button" className="secondary" onClick={exportExcel}><Download size={16} /> Excel</button>
          <button type="button" className="secondary" onClick={exportCSV}><Download size={16} /> CSV</button>
          <button type="button" className="secondary" onClick={printReport}><Download size={16} /> PDF / Print</button>
        </div>
      </div>
      ...
      <div className="report-print-area">
        <div className="report-heading">
          <div>
            <p className="eyebrow">ANNOTATEPRO REPORT</p>
            <h2>{reportTitle}</h2>
            <p className="muted">Reporting period: <b>{range.label}</b></p>
          </div>
          <div className="report-status-note">
            {selectedSheetRows.length || selectedAccuracyRows.length
              ? "Data available"
        ...
        <div className="cards">
          <Metric icon={Users} label="Team members" value={summary.people} note="Members with records" trend="Report" />
          <Metric icon={ImageIcon} label="Images worked" value={summary.images.toLocaleString()} note="Daily Effort" trend="Live" />
          <Metric icon={ClipboardCheck} label="Reviews" value={summary.reviews.toLocaleString()} note="Review activity" trend="Live" />
          <Metric icon={CheckCircle2} label="Accuracy" value={selectedAccuracyRows.length ? `${summary.accuracy.toFixed(1)}%` : "—"} note="Accuracy Report" trend={selectedAccuracyRows.length ? "Good" : "Pending"} />
        </div>

        <div className="grid two">
          <Panel title="QA summary">
            <div className="qa-grid">
              <div><h2>{summary.tp.toLocaleString()}</h2><p className="muted">True positives</p></div>
              <div><h2>{summary.fp.toLocaleString()}</h2><p className="muted">False positives</p></div>
              <div><h2>{summary.fn.toLocaleString()}</h2><p className="muted">False negatives</p></div>
              <div><h2>{summary.errors.toLocaleString()}</h2><p className="muted">Total errors</p></div>
              <div><h2>{selectedAccuracyRows.length ? summary.score.toFixed(1) : "—"}</h2><p className="muted">Average score</p></div>
              <div><h2>{summary.dailyCount.toLocaleString()}</h2><p className="muted">Accuracy daily count</p></div>
            </div>
          </Panel>

          <Panel title="Project summary">
            {!projectSummary.length ? (
              <p className="muted">No project work exists for <b>{range.label}</b>.</p>
            ) : (
              <div className="project-list">
                {projectSummary.slice(0, 8).map(row => (
                  <div className="project-row" key={row.project}>
                    <div className="project-icon"><FolderKanban size={18} /></div>
                    <div className="grow">
                      <div className="row-title"><b>{row.project}</b><span>{row.images.toLocaleString()}</span></div>
                      <Progress value={summary.images ? (row.images / summary.images) * 100 : 0} />
                      <small>{row.people} team member{row.people === 1 ? "" : "s"}</small>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <Panel title={`Team performance — ${range.label}`}>
          {!reportRows.length ? (
            <p className="muted">No report data exists for <b>{range.label}</b>.</p>
          ) : (
            <div className="table-wrap">
              <table>
                <thead>
                ...
                <tbody>
                  {reportRows.map((row, index) => (
                    <tr key={`${row.name}-${index}`}>
                      ...
                      <td>{row.dailyCount.toLocaleString()}</td>
                      <td>{row.accuracy ? `${row.accuracy.toFixed(1)}%` : "—"}</td>
        ...
        <Panel title={`Report details — ${range.label}`}>
          <div className="qa-grid">
            <div><h2>{summary.people}</h2><p className="muted">Team members</p></div>
            <div><h2>{summary.projects}</h2><p className="muted">Projects</p></div>
            <div><h2>{summary.images.toLocaleString()}</h2><p className="muted">Images worked</p></div>
            <div><h2>{summary.reviews.toLocaleString()}</h2><p className="muted">Reviews</p></div>
            <div><h2>{summary.errors.toLocaleString()}</h2><p className="muted">QA errors</p></div>
            <div><h2>{selectedAccuracyRows.length ? `${summary.accuracy.toFixed(1)}%` : "—"}</h2><p className="muted">Accuracy</p></div>
          </div>
        </Panel>
      </div>
    </Page>
